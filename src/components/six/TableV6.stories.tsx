...
export const Primary: Story = {
  render: () => {
    ...
    return (
      <Container>
        <Header>
          <Title>📊 Urine Drug Test Results (Laboratory Test)</Title>
          <Controls>
            <ViewToggleContainer>
              <ViewToggleButton 
...
const ChartBox = styled.div.attrs({
  role: 'complementary'
})`
  padding: 16px;
  background: white;
...
const CategoryRow = styled(Row).attrs({
  role: 'row'
})`
  font-weight: bold;
`

const DataRow = styled(Row).attrs({
  role: 'row'
})`
  &:hover {
    background: #f8f9fa;
...
const TotalRow = styled(Row).attrs({
  role: 'row'
})`
  font-weight: bold;
