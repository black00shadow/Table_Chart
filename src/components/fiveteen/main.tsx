...
const TitleIcon = styled.div`
  width: 65px;
  height: 51px;
  background-image: url(${require('@/assets/img/4.png')});
  cursor: pointer;
  margin-right: 10px;
`
...
const Discover: React.FC = () => {
  ...
  >([])
  const [tableCenterData, setTableCenterData] = useState<
    {
      name: string
      data: number[]
    }[][]
  ...
  const loadData = async () => {
    ...
    setTableCenterData(res.data.center)
  }
  return (
    <Container>
      <MainContent>
        <Header>
          <TitleIcon />
          <PageTitle>Patient Communication </PageTitle>
          <Options>
            <Space>
              {/* <Button
        ...
        {displayMode === 'table' ? (
          ...
          <ChartView
            timeRange={timeRange}
            tableData={tableData}
            tableCenterData={tableCenterData}
          />
        )}
      </MainContent>
    </Container>
