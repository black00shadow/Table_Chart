...
const Table: React.FC<Props> = ({ timeRange, tableData, contentData }) => {
  // 使用useState管理图表数据
    ...
    item.forEach((it, id) => {
      if (timeRange === it.time) {
        it.info.forEach((i, idx) => {
          contentData[index].dataName.push(i.name)
          contentData[index].series.push({
            name: i.name,
            data: i.data
          })
        })
            ...
            return (
              <>
                <MainCcontent>
                  <LeftContent1>
                    {
                      item.content != "" ? <Arrow content={item.content} backgroundColor="#176CC9" /> : <></>
                    }
