...
export const Primary: Story = {
  render: () => {
    ...
    // 计算总计
    const calculateTotal = (field: 'today' | 'week' | 'month' | 'year') => {
      return mockData.reduce((sum, item) => sum + item[field], 0)
    }

    return (
      <Container>
        <Header>
          <Title>📊 Hair Drug Test (Laboratory Test)</Title>
          <Controls>
            <ViewToggleContainer>
              <ViewToggleButton 
        ...
        {activeView === 'data' && (
          <TableContainer data-testid="table-container">
            ...
            <TotalRow>
              ...
              <Cell>{calculateTotal('today')}</Cell>
              <Cell>{calculateTotal('week')}</Cell>
              <Cell>{calculateTotal('month')}</Cell>
              <Cell>{calculateTotal('year')}</Cell>
            </TotalRow>
          </TableContainer>
        )}
