...
export const Primary: Story = {
  render: () => {
    ...
    const getChartData = (period: 'today' | 'week' | 'month' | 'year') => {
      ...
      mockData.forEach(category => {
        category.items.forEach(item => {
          result.push({
            name: `${category.category} - ${item.name}`,
            value: item[period]
          })
        })
      })
    ...
    return (
      <Container>
        <Header>
          <Title>📊 Drug Classes - Negative Results</Title>
          <Controls>
            <ViewToggleContainer>
              <ViewToggleButton 
            ...
            {mockData.map((category, categoryIndex) => (
              <CategorySection key={categoryIndex}>
                ...
                {category.items.map((item, itemIndex) => (
                  <DataRow key={itemIndex}>
                    <Cell style={{ width: '200px', background: '#e6f3ff' }}>{item.name}</Cell>
